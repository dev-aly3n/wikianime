...
const Top = ({ anime, rank }) => {
  ...
  return (
    <div  className="relative group transform hover:translate-x-3 hover:translate-y-1 transition-all duration-1000
    hover:shadow-2xl">
    <motion.a
      href={`/anime/${anime.id}`}
    ...
    >
      ...
      <div className="flex flex-row">
        ...
        <div className="hidden lg:flex ml-5 self-center  w-72 h-10 break-words lg:line-clamp-2 text-sm">
        <Markup content={anime.description} />
      </div>
    ...
    <span className="rounded-full bg-indigo-600 text-white absolute -top-3 -left-3 w-9 h-9 text-xl flex justify-center items-center
    ">{rank}</span>
    </div>
  );
};
